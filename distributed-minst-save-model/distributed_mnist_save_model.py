...
def train(server, log_dir, context):
    training_iteration = context.get('training_iteration') or 1000
    model_version = context.get('model_version') or 1
    work_dir = context.get('work_dir') or '{}/mnist'.format(log_dir)
    source_url = context.get('source_url') or 'file:///mnt/mesos/sandbox/cvdf-datasets/mnist/'
    export_path_base = context.get('export_path_base') or log_dir
    ...
    with tf.device(tf.train.replica_device_setter(
        ...
        mnist = input_data.read_data_sets(work_dir, one_hot=True, source_url=source_url)
